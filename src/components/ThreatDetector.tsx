...
export const detectThreats = (logs: string[]): Threat[] => {
  ...
  logs.forEach((log, index) => {
    ...
    for (const { pattern, type, desc, mitigation } of MALWARE_PATTERNS.critical) {
      if (pattern.test(log)) {
        threats.push({
    ...
    for (const { pattern, type, desc, mitigation } of MALWARE_PATTERNS.warning) {
      if (pattern.test(log)) {
        threats.push({
          id: `threat-${Date.now()}-${index}`,
          severity: "warning",
    ...
    // Check info patterns
    for (const { pattern, type, desc, mitigation } of MALWARE_PATTERNS.info) {
      if (pattern.test(log)) {
        threats.push({
          id: `threat-${Date.now()}-${index}`,
          severity: "info",
